...
2. Create/verify a `staging` dataset for local content wiring
3. Copy the `projectId` + dataset into `studio/.env.local` (see `.env.example`)
...
- **New page** : add a slug to `PAGE_IDS` in `deskStructure.ts`, then seed
  the document with `pnpm sanity:push --type page --id page-<slug> --data <json>`.
