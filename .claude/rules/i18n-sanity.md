...
# i18n + Sanity — Protocole contenu (path-triggered)
...
**Chargement** : déclenchée quand on touche `src/**/*.tsx`, `src/locales/**`,
les helpers `src/lib/i18n*` ou `src/lib/sanity*`, ou l'arbre `studio/**`.
Le résumé critique (taxonomie, FR interdit en JSX, zéro champ vide) est
toujours visible via `critical.md` §7.
...
| 9   | Aucun champ éditable non-locale dans un doc Sanity         | `scripts/validate-sanity-schema.js`                               |
| 10  | Singletons = actions filtrées (no create/delete/duplicate) | `sanity.config.ts` `document.actions` filter                      |
| 11  | Description humaine sur chaque field Sanity                | review de schéma + `validate:sanity-schema`                       |
| 12  | StudioLayout avec CSS fixes anti-UX pénibles               | Composant importé tel quel d'étoiles                              |
| 13  | Tout champ Sanity DOIT avoir les 3 locales remplies        | `scripts/validate-sanity-content.js` + CI gate                    |
...
**HARD GATE (incident 2026-04-23)** : si `client.md` est en état placeholder
(`Status: 🟡 Placeholder` ou champs `Identité > Nom:` vides), **toute**
rédaction de contenu visible utilisateur est interdite, pas seulement
`/wire-content` :

- Interdit : adresses, téléphones, emails, noms de partenaires, noms de
  fondateurs, paragraphes marketing, SEO descriptions, alt text signifiant.
- Autorisé : strings d'UI purement techniques (labels boutons, états
  système, placeholders d'input).

Claude doit **refuser** ou **émettre un ACTION HUMAINE REQUISE** pour
faire remplir `client.md` avant de procéder. Le coût d'inventer du
contenu est irrécupérable : l'owner passe au crible, trouve du fantôme,
et la confiance dans le livrable s'effondre.

Jamais poussé en prod, jamais exposé côté client.

Avant livraison client, `pnpm validate:sanity-content:required` doit passer.
Contrairement à `pnpm validate`, ce mode échoue si Sanity/env/réseau manque.
