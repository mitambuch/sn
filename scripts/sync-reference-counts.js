...
/* ═══════════════════════════════════════════════════════════════
   ...
   3. README count phrases (detect-only, check mode fails)
      ...
      The README also mentions agent count, but this script does not
      validate that phrase yet.
...
const commandFiles = listFiles(join(ROOT, '.claude/commands'), n => n.endsWith('.md'));
const commandsCount = commandFiles.length;
const commandNames = new Set(commandFiles.map(n => n.replace(/\.md$/, '')));

const agentsCount = countFiles(join(ROOT, '.claude/agents'), n => n.endsWith('.md'));

const ruleFiles = listFiles(join(ROOT, '.claude/rules'), n => n.endsWith('.md'));
...
for (const { heading, count } of headingSections) {
  ...
  if (!match) {
    if (referenceContent) {
      headingDrift.push(`- REFERENCE heading missing: "## ${heading} (N)" — expected ${count}`);
    }
    continue;
  }
...
if (referenceContent) {
  ...
  if (cmdSection) {
    ...
    if (missing.length)
      tableDrift.push(
        `- REFERENCE commands table missing: ${missing.map(n => `/${n}`).join(', ')}`,
      );
    if (extra.length)
      tableDrift.push(
        `- REFERENCE commands table has orphans: ${extra.map(n => `/${n}`).join(', ')}`,
      );
  }
