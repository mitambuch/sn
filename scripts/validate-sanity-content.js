...
/* ═══════════════════════════════════════════════════════════════
   ...
   Usage:
     ...
     node scripts/validate-sanity-content.js --required
   ═══════════════════════════════════════════════════════════════ */
...
const DATASET = process.env.VITE_SANITY_DATASET || process.env.SANITY_STUDIO_DATASET || 'staging';
const API_VERSION = process.env.VITE_SANITY_API_VERSION || '2024-06-01';
const REQUIRED = process.argv.includes('--required') || process.env.SANITY_CONTENT_REQUIRED === '1';
...
function skipOrFail(reason) {
  ...
  console.log(`  ⓘ ${reason} — skipping content validation.`);
  process.exit(0);
}

if (!PROJECT_ID) {
  skipOrFail('Sanity not configured (VITE_SANITY_PROJECT_ID empty)');
}
...
async function main() {
  ...
  try {
    ...
    skipOrFail(`Cannot reach Sanity (${err.message})`);
  }

  if (!res.ok) {
    skipOrFail(`Sanity returned HTTP ${res.status}`);
  }
  ...
  if (missingDocs.length > 0) {
    console.error(
      `  ✗ ${missingDocs.length} canonical singleton(s) not found in dataset ${DATASET}:`,
    );
...
main().catch(err => {
  skipOrFail(`validate-sanity-content failed unexpectedly (${err.message})`);
});
