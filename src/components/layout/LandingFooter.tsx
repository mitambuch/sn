    ...
    >
      <div className="relative mx-auto w-full max-w-400 px-5 md:px-6">
        ...
        <div className="mt-12 grid grid-cols-1 gap-4 md:mt-16 md:grid-cols-3 md:gap-5">
          {CTAS.map((cta, index) => (
            <a
              ...
              className="border-fg/40 text-fg hover:border-fg group focus-visible:ring-fg/30 relative flex min-h-36 flex-col justify-between rounded-sm border p-6 transition-colors duration-200 focus-visible:ring-2 focus-visible:outline-none md:p-7"
            >
              <span className="text-fg font-mono text-[10px] font-semibold tracking-[0.4em] uppercase tabular-nums">
                0{index + 1} / {cta.label}
              </span>
              <div className="mt-8 flex items-end justify-between gap-4">
                <span className="text-fg font-mono text-base leading-tight font-semibold tracking-tight md:text-lg">
                  {cta.value}
                </span>
                <ArrowUpRight
                  size={20}
                  strokeWidth={1.6}
                  aria-hidden="true"
                  className="text-fg shrink-0 transition-transform duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5"
                />
              </div>
            </a>
          ))}
        ...
        <Link
          ...
          className="border-fg/40 text-fg hover:border-fg group focus-visible:ring-fg/30 mt-5 grid grid-cols-1 gap-8 rounded-sm border p-7 transition-colors duration-300 focus-visible:ring-2 focus-visible:outline-none md:mt-6 md:grid-cols-[1fr_auto] md:items-end md:gap-10 md:p-9"
        >
          <div className="flex flex-col gap-3">
            <span className="text-fg font-mono text-[10px] font-semibold tracking-[0.4em] uppercase">
              Espace client
            </span>
            <span className="text-fg font-mono text-2xl leading-[1.05] font-semibold tracking-tight uppercase md:text-4xl lg:text-5xl">
              Déjà client ?
              <br />
              Accédez à votre dossier privé.
            </span>
            <span className="text-fg mt-3 max-w-md text-base leading-relaxed md:mt-4">
              Correspondances, suivi des demandes en cours, documents partagés — tout au même
              endroit, en stricte confidentialité.
            </span>
          </div>
          <span className="border-fg/40 text-fg group-hover:bg-fg group-hover:text-bg group-hover:border-fg inline-flex items-center justify-between gap-4 rounded-sm border px-6 py-4 font-mono text-[10px] font-semibold tracking-[0.4em] uppercase transition-colors duration-200 md:min-w-56 md:justify-center md:gap-3">
        ...
        <div className="border-fg/20 mt-20 flex flex-col gap-6 border-t pt-8 md:mt-28 md:flex-row md:flex-wrap md:items-center md:justify-between md:gap-8">
          <button
            ...
            className="text-fg focus-visible:ring-fg/30 group inline-flex flex-col items-start gap-4 rounded-sm focus-visible:ring-2 focus-visible:outline-none"
          >
            <Wordmark className="h-5 w-auto transition-transform duration-300 group-hover:-translate-y-0.5 md:h-6" />
