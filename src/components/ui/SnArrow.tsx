// ═══════════════════════════════════════════════════
// SnArrow — animated S↗N signature with breathing arrow
//
// WHAT: Same vector as <Logomark /> but the middle arrow path pulses
//       slowly (3.5s loop) and the whole mark draws in via clip-path
//       on mount. The animation respects prefers-reduced-motion via
//       the global rule in animations.css.
// WHEN: Brand corners that need to feel alive — top-bar of the
//       landing, invite page seal, footer end-cap. For a static mark
//       use <Logomark /> instead.
// CHANGE PULSE SPEED: edit @keyframes arrow-pulse in animations.css.
// CHANGE DRAW-IN DURATION: edit @keyframes wordmark-draw.
// ═══════════════════════════════════════════════════

import { cn } from '@utils/cn';

interface SnArrowProps {
  className?: string;
  ariaLabel?: string;
  /** Disable the breathing pulse on the arrow (still draws in). */
  staticArrow?: boolean;
}

/** Animated S↗N signature. Subtle pulse on the arrow + clip-path draw-in. */
export const SnArrow = ({ className, ariaLabel = 'SAW Next', staticArrow }: SnArrowProps) => {
  return (
    <svg
      ...
      className={cn('animate-wordmark-draw h-auto w-full fill-current', className)}
    >
      ...
      <polygon
        className={staticArrow ? undefined : 'animate-arrow-pulse'}
      ...
      />
      <polygon points="188.64 1.92 188.64 63.12 162.84 1.92 146.88 1.92 146.88 87.12 161.28 87.12 161.28 25.92 187.08 87.12 203.04 87.12 203.04 1.92 188.64 1.92" />
    </svg>
  );
};
