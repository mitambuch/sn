...
| **worker-haiku** | R0 executor (rename, regen, bump, memory ops) — Haiku 4.5, fast + cheap |
| **worker-sonnet** | R1 executor (new component/page/field, bug fix with repro) — Sonnet 4.6 |
...
| Rule | Load | Scope |
|------|------|-------|
| `anti-complaisance.md` | **always** | Franchise before flattery — forbidden openers, CHALLENGE block before R2, calibrated honesty over grumpiness |
| `creative-ambition.md` | **always** | Mandates ≥ 2 mechanics from `creative-library/mechanics/` in every VISION — no vague "animation/hover"; catalogue slug references required |
| `critical.md` | **always** | Fourteen non-negotiables: branch-first, memory consult, release check, Karpathy, conducteur/navigateur, validate-before-commit, content architecture, proactive dispatch, /clear discipline, anti-complaisance, CHALLENGE block, DISPATCH_CLASS hint, vision-first, creative-ambition |
| `dispatch.md` | `.claude/agents/**`, `.claude/commands/dispatch.md`, `.claude/rules/dispatch.md`, `.claude/rules/intent-routing.md` | R0/R1/R2 taxonomy + parallel dispatch rules + Agent invocation mechanic — Conductor & Workers protocol |
| `intent-routing.md` | **always** | Natural-language → skill/agent auto-invocation — owner never types `/`, Claude detects intent + invokes Skill |
| `i18n-sanity.md` | `src/**/*.tsx`, `src/locales/**`, `src/lib/i18n*`, `src/lib/sanity*`, `studio/**` | Taxonomy (inline/menu/singleton), i18n-vs-sanity arbitrage, 13 HDVA lessons, zero-empty-field rule, content-from-Claude workflow |
...
| `time-awareness.md` | **always** | Real-clock injection via UserPromptSubmit hook, duration estimation anchored to measured data, deadline tracking, audit trail protocol |
| `vision-first.md` | **always** | Mandatory VISION block (Intent/Refs/Keep/Drop/Verdict) before any non-trivial UI pass — prevents direction drift across passes |
| `ux-first.md` | **always** | Mandatory UX block (Goal/Friction/Micro-feature signature/Share hook/Métrique) before any user-facing page or section — one slug from `creative-library/interactions/` required |
