...
# 2. In Claude Code — design direction (runs FIRST, feeds /init)
/brief                      # 5 questions → writes Design Direction + Composition Rules to CLAUDE.md

# 3. In Claude Code — project identity & styling (reads /brief output)
...
| `/init`      | Identity + styling: colors, fonts, vibe, content (reads `/brief` output) | src/index.css, site.ts, fonts.css, Home.tsx, manifest.json                   |
