...
| `VITE_DEFAULT_LOCALE`        | `fr`                    | First-visit locale fallback                                               |
| `VITE_SANITY_PROJECT_ID`     | (empty)                 | Enables Sanity-backed content reads                                       |
| `VITE_SANITY_DATASET`        | `staging`               | Local content dataset; deploy should use `production`                     |
| `VITE_SANITY_API_VERSION`    | `2024-06-01`            | Sanity API version                                                        |
| `SANITY_WRITE_TOKEN`         | (empty)                 | Server-side write token for `pnpm sanity:push`; never prefix with `VITE_` |
